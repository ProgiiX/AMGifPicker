...
extension Storage: StorageAware {
  ...
  public func removeExpiredObjects() throws {
   ...
   public func makeFilePath(_ key: String) -> String? {
        if let disk = (interalStorage as? TypeWrapperStorage)?.internalStorage as? DiskStorage {
            return disk.makeFilePath(for: key)
        }
        return nil
    }
}
